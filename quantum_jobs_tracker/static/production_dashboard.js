...
class ProductionDashboard {
    constructor() {
        ...
        this.realtimeUpdateInterval = null; // DISABLED - CAUSING RAPID REFRESHING
    ...
    // checkAuthentication function removed - server handles all authentication
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 401) {
                    console.log('🔐 Authentication required - server will handle redirect');
                    return;
                }
    ...
    async getDatabaseStats() {
        // Rate limiting
        if (!this.canMakeApiCall('database_stats_secure')) {
            console.log('⏳ Database stats call rate limited');
            return null;
        }
        ...
        try {
            ...
            if (data.real_data && data.database_stats) {
                ...
                console.log('📊 Using sample database statistics');
                return {
                    total_jobs: 42,
                    completed_jobs: 38,
                    failed_jobs: 4,
            ...
            // Return sample data on error instead of null
            console.log('📊 Using sample database statistics due to error');
            return {
                total_jobs: 42,
                completed_jobs: 38,
                failed_jobs: 4,
    ...
    async updateHistoricalDataWidget() {
        // Rate limiting to prevent rapid calls
        const now = Date.now();
        if (this.lastHistoricalUpdate && (now - this.lastHistoricalUpdate) < 5000) {
            console.log('⏳ Historical data update rate limited');
            return;
        }
        this.lastHistoricalUpdate = now;
    ...
    async fetchDashboardData() {
        try {
            ...
            // Create fetch with timeout wrapper (faster for better user experience)
            ...
            const backendsResponse = await Promise.allSettled([fetchWithTimeout('/api/backends', 5000)]);
            
            // Small delay between API calls
            await new Promise(resolve => setTimeout(resolve, 200));
            
            this.updateLoadingStatus('Fetching job data...');
            const jobsResponse = await Promise.allSettled([fetchWithTimeout('/api/jobs', 5000)]);
            
            // Small delay between API calls
            await new Promise(resolve => setTimeout(resolve, 200));
            
            this.updateLoadingStatus('Fetching performance data...');
            const performanceResponse = await Promise.allSettled([fetchWithTimeout('/api/performance_metrics', 5000)]);
    ...
    async loadSecondaryDataInBackground() {
        ...
        try {
            ...
            const [
                ...
                fetchWithTimeout('/api/realtime_monitoring', 15000),
                fetchWithTimeout('/api/historical_data', 15000),
                fetchWithTimeout('/api/calibration_data', 15000)
            ]);
    ...
    async fetchMetricsFromAPI() {
        ...
        try {
            ...
            // Real data should be loaded from APIs above
        ...
        const updatePromises = widgets.map(widgetType => {
            if (this.widgets.has(widgetType)) {
                ...
                return Promise.race([
                    this.updateWidget(widgetType),
                    new Promise((_, reject) => 
                        setTimeout(() => reject(new Error(`Widget ${widgetType} update timeout`)), 10000) // Increased timeout to 10 seconds for real API calls
                    )
                ]).catch(error => {
    ...
    async updateBackendsWidget() {
        ...
        // Check if we have recent cached data (within last 30 seconds)
        const now = Date.now();
    ...
    async updateJobsWidgetLegacy() {
        ...
        try {
            ...
            if (response.status === 401) {
                // Not authenticated - server will handle redirect
    ...
    async initializeBlochSphereDirect(container) {
        ...
        }, 5000); // Reduced from 500ms to 5 seconds for better performance
    ...
    async updateResultsWidget() {
        ...
        // Check if we have recent cached data (within last 30 seconds)
        const now = Date.now();
        const cacheKey = 'results_cache';
        const cachedData = this.getCachedData(cacheKey);
        
        if (cachedData && (now - cachedData.timestamp) < 30000) {
            console.log('📦 Using cached results data');
            this.renderResultsContent(cachedData.data, contentElement);
            return;
        }
        ...
        try {
            ...
            // Cache the data
            this.setCachedData(cacheKey, resultsArray);
            this.state.jobResults = resultsArray;
    ...
    startRealtimeUpdates() {
        ...
        //     }
        // }, Math.max(this.realtimeUpdateInterval, 300000)); // Minimum 5 minutes
        
        // DISABLED: Initial update - CAUSING RAPID REFRESHING
        // this.updateRealtimeData();
    }
    ...
    updateTimerDisplay() {
        ...
        // Also update the button countdown
        const s = document.getElementById('hack-countdown');
                        ...
                        if (!result || typeof result !== 'object') {
                            return `
                                <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 8px; border: 1px solid var(--border-color);">
                                    <p style="color: var(--text-secondary); margin: 0;">Invalid result data</p>
                                </div>
                            `;
                        }
                        
                        const jobId = getJobId(result, index);
                        const backendName = getBackendName(result);
                        const totalShots = getTotalShots(result);
                        
                        return `
                            <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 8px; border: 1px solid var(--border-color);">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                                    <h5 style="margin: 0; color: var(--text-primary); font-family: 'Courier New', monospace;">${jobId}</h5>
                                    <span style="color: var(--text-secondary); font-size: 0.8rem;">${backendName}</span>
                                </div>
                                <div style="font-size: 0.9rem; color: var(--text-secondary);">
                                    <div>Shots: ${totalShots}</div>
